...
- 歌詞同步顯示
- 連續播放模式
  - 當取得的歌曲數量超過 1 首時，會隨機選歌播放。
- 專輯封面顯示：
  - MP3 的 ID3 標籤需包含專輯圖檔
  - 需搭載 PSRAM 才會顯示

***註：啟動裝置時為「單曲模式」，可用語音切換為「連續模式」。此設定值會儲存在 NVS。***
...
**4. ESP32-C6 可考慮捨棄螢幕顯示功能：**

如果是用的是 ESP32-C6 最陽春的開發版（例如[這種](https://www.waveshare.net/shop/ESP32-C6-Zero.htm)），可考慮捨棄屏幕顯示功能：
...
- Navidrome 支援歌詞同步，歌詞形式可用 ID3 內嵌（`LYRICS` 或 `UNSYNCEDLYRICS` 欄位）或外掛 .lrc 檔案。
  - 外掛歌詞僅須將 .lrc 歌詞文件與 .mp3 放在同一目錄下，使用相同主檔名即可。

- Navidrome 依賴 mp3 音樂檔案的 ID3 標籤進行建檔與搜尋，請確保 ID3 標籤 artist（歌手）和 title（歌名）欄位填寫正確完整（可使用 [Mp3tag](https://www.mp3tag.de/en/download.html) 或其他 ID3 工具編輯）。
